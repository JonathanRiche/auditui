...
fn writeHealth(writer: *std.Io.Writer) !void {
    try std.json.Stringify.value(.{ .status = "ok", .engine = "audible-zig", .version = "0.1.0", .protocolVersion = engine.rpc.version }, .{}, writer);
...
fn run(init: std.process.Init) !void {
    ...
    if (std.mem.eql(u8, args[1], "--version")) return stdout.writeAll("audible-zig 0.1.0\n");
